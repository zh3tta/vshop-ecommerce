      ...
      <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            ...
            <tbody>
              <tr v-for="category in categories.data" :key="category.id" class="border-b dark:border-gray-700">
                ...
                <td class="px-4 flex items-center justify-end">
                  <div :id="`${category.id}`" class="col-span-2 content-center sm:col-span-1 sm:justify-self-end">
                    <div class="py-1 inline-flex m-1" :aria-labelledby="`${category.id}-button`">
        ...
        <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
          <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
            Showing
            <span class="font-semibold text-gray-900 dark:text-white">{{ categories.from }}-{{ categories.to }}</span>
            of
            <span class="font-semibold text-gray-900 dark:text-white">{{ categories.total }}</span>
          </span>
          <ul class="inline-flex items-stretch -space-x-px">
            <li v-if="categories.prev_page_url">
              <a @click.prevent="changePage(categories.prev_page_url)"
            ...
            <li v-for="page in categories.links" :key="page.label">
            ...
            <li v-if="categories.next_page_url">
              <a @click.prevent="changePage(categories.next_page_url)" 
