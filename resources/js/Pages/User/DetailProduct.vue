...
const addToCart = (detail) => {
  router.post(route('cart.store', detail), {
    onSuccess: (page) => {
...
<template>
  <UserLayouts>
    <section class="py-8 bg-white md:py-16 dark:bg-gray-900 antialiased">
      <div v-for="detail in details" :key="detail.id" class="max-w-screen-xl px-4 mx-auto 2xl:px-0">
        <div class="lg:grid lg:grid-cols-2 lg:gap-8 xl:gap-16">
          <div class="shrink-0 max-w-md lg:max-w-lg mx-auto">
            <img v-if="detail.product_images.length > 0" :src="`/${detail.product_images[0].image}`" :alt="detail.imageAlt" class="h-full w-full object-cover object-center lg:h-full lg:w-full" />
            <img v-else src="https://upload.wikimedia.org/wikipedia/commons/thumb/6/65/No-Image-Placeholder.svg/330px-No-Image-Placeholder.svg.png" :alt="detail.imageAlt" 
              class="h-full w-full object-cover object-center lg:h-full lg:w-full" />
          </div>
          <div class="mt-6 sm:mt-8 lg:mt-0">
            <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">
              {{ detail.title }} - {{ detail.brand.name }}
            </h1>
